...
def load_data(f_ins: list, t_reso: float, x_reso: float, source_type: str = '1', trig_preview=False, **kwargs):
    ...
    Arguments:
        ...
        trig_preview: visualize the raw data. Only works for 2D data, i.e. len(f_ins) > 1. 
    Optional arguments:
    ...
    if trig_preview:
        if source_mode in ['1', 'mean', 'max']:
            print("Preview skipped since only one file is provided.")
        else:
            fig, axes = plt.subplots(1,2, figsize = (11, 5))
            # 2D preview
            X, Y = np.meshgrid(x, y, indexing='ij')
            axes[0].pcolormesh(X, Y, np.sum(data_raw, axis=0), cmap='viridis')
            # add circles for radial mode
            if source_type == 'radial':
                r_display = r[::max(1, len(r)//5)]  # picks ~5 evenly spaced radii
                for ri in r_display:
...
#%% config
##### data params #####
source_mode = 'interp'  # how to get 1D data from 2D raw. allow: "1", "mean", "max", "radial", "interp"
f_in = None  # path to the .dat file. if None, will prompt user to select file
t_step = 0.016  # time step in ns
motor_step_x = 10  # motor step in um
motor_step_y = 10  # motor step in um. Ignored in 1D, max or mean mode.
mag = 182  # microscopy magnification
trig_447_replica_fix = False  # When 447 laser is used, it sometimes create 4 replica. This trigger will align and sum-over the 4 replica.
replica_n = 4 # number of replica.
min_dt = 5  # minimum dt between replica, in ns.
theta = 45 / 180 * np.pi  # angle of interpolation line in radian. Only used for interpolation.
...
representative_t = [0, 5, 10, 40]     # representative frames to be plotted.
##### output params #####
...
params_data = hyconfig.code_section({
    ...
    "motor_step": motor_step_x,  # motor step in um
...
elif source_mode in ['interp']:
    data, t, x, fig_preview, axes_preview = load_data(f_in, t_step, motor_step_x / mag, source_type=source_mode, y_reso = motor_step_y / mag, theta = theta, trig_preview=True)
    plt.savefig(f"{dir_out}\\data_preview.png")
elif source_mode in ['radial']:
    data, t, x, fig_preview, axes_preview = load_data(f_in, t_step, motor_step_x / mag, source_type=source_mode, y_reso = motor_step_y / mag, theta = theta, trig_preview=True)
    plt.savefig(f"{dir_out}\\data_preview.png")
